...
int     check_time(double s_time, t_option *o)
{
    struct timeval tv;
    double begin, end;
...
void    *t_function(void *data)
{
    ...
    while (1)
    {
        ...
        if (info.status == THINK)
        {
            ...
            int l = info.id - 1;
            int r = info.id + 1;
            if (info.id - 1 % o->number_of_philo < 0)
            {
                if (o->fork[o->number_of_philo - 1] == 0)
                {
                }
            }
        ...
        // 자는 시간 체크
        if (info.status == SLEEP && check_time(s_time, o) == 0)
        {
            info.status = THINK;
            write(1, "think\n", 6);
        }
