...
private:
	typedef __tree<key_type, value_type, ft::select_first<value_type>, key_compare, allocator_type>
			__base;
	__base __tree_;
...
public:
 ...
 public:
  ...
  map(_InputIterator first, _InputIterator last, const key_compare& comp = key_compare(),
      const allocator_type& alloc = allocator_type()) : __tree_(comp, alloc) {
  ...
  // iterators
  iterator begin() throw() { return iterator(__tree_.begin()); }
  const_iterator begin() const throw() { return const_iterator(__tree_.begin()); }
  iterator end() throw() { return iterator(__tree_.end()); }
  const_iterator end() const throw() { return const_iterator(__tree_.end()); }
  reverse_iterator rbegin() throw() { return __tree_.rbegin(); }
  const_reverse_iterator rbegin() const throw() { return __tree_.rbegin(); }
  reverse_iterator rend() throw() { return __tree_.rend(); }
  const_reverse_iterator rend() const throw() { return __tree_.rend(); }

  // capacity
  bool empty() const throw() { return __tree_.empty(); }
...
template <typename _Key, typename _T, typename _Compare, typename _Alloc>
void swap(ft::map<_Key, _T, _Compare, _Alloc>& lhs, ft::map<_Key, _T, _Compare, _Alloc>& rhs) {
	lhs.swap(rhs);
}
}
